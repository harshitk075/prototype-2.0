...
def parseArgs():
    ...
    parser.add_argument("image_path", help = "Path to the input image to be processed, relative to {}".format(os.getcwd()))
    ...
    with detection_graph.as_default():
        with tf.Session() as sess:
            while True:
                ...
                ###OCR-START - Comment the above cell (OCR ALTERNATIVE) if running this code section. Uncomment otherwise.
                
                    im = Image.fromarray(image_np_original,'RGB')
                    image = im
                #       image = Image.open(set_image_dpi(im))
                    ocr_output = pytesseract.image_to_data(im, output_type=pytesseract.Output.DATAFRAME)
                    #print(ocr_output)
                    width, height = image.size
                    ocr_output[['left','width']] /= width
                    ocr_output[['top','height']] /= height
